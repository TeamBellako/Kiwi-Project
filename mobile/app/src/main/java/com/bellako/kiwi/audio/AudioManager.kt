...
class AudioLayerPlayer(var isActive: Boolean, val player: ExoPlayer)


object AudioManager {

    private var globalVolumeMusic: Float = 1f

    private var globalVolumeSFX: Float = 1f
    ...
    fun stopMusic(resId: Int, fadeDuration: Long = 3000) {
        if (currentMusic.contains(resId)) {
            stop(currentMusic[resId]!!.player, fadeDuration)
        }
    }
        ...
        if (type == AudioType.MUSIC) {
            for ((currentMusicResId, currentMusicPlayer) in currentMusic) {
                if (!layers.contains(AudioLayer(currentMusicResId, true))) {
                    stop(currentMusicPlayer.player, fadeDuration)
                }
            }
        }
        for (layer in layers) {
            if (type == AudioType.MUSIC && currentMusic.contains(layer.resId)) {
                ...
                if (player.isActive != layer.isActive) {
                    ...
                    fade(player.player, fromVolume, toVolume, fadeDuration)
                }
            } else {
    ...
    private fun addLayer(context: Context, layer: AudioLayer, type: AudioType, fadeDuration: Long) {
        ...
        fade(player, 0f, if (layer.isActive) actualToVolume else 0f, if (layer.isActive) fadeDuration else 0)
        player.play()
        currentMusic[layer.resId] = AudioLayerPlayer(layer.isActive, player)
    }

    private fun stop(player: ExoPlayer, fadeDuration: Long) {
        fade(player, 1f, 0f, fadeDuration) {
            player.stop()
        }
    }

    private fun fade(player: ExoPlayer, fromVolume: Float, toVolume: Float, duration: Long, onComplete: (() -> Unit)? = null) {
        val actualFromVolume = fromVolume.coerceIn(0f, 1f)
        val actualToVolume = toVolume.coerceIn(0f, 1f)
        if (duration <= 0) {
            player.volume = actualToVolume
            onComplete?.invoke()
            return
        }
        player.volume = actualFromVolume
        ...
        val runnable = object : Runnable {
            override fun run() {
                if (currentStep <= steps) {
                    player.volume = actualFromVolume + volumeDelta * currentStep
                    currentStep++
    ...
    private fun updateCurrentMusicVolume() {
        for ((currentMusicResId, currentMusicPlayer) in currentMusic) {
            if (currentMusicPlayer.isActive) {
                fade(currentMusicPlayer.player, currentMusicPlayer.player.volume, globalVolumeMusic, 250)
            }
        }
    }
