-- The whole body is guarded on `suggested_goals` still existing: that table is
-- renamed to `_old_suggested_goals` and dropped at the end of this migration,
-- so its presence is a reliable signal that the refactor has not been applied
-- yet. A re-run after a successful migration becomes a no-op.
DROP PROCEDURE IF EXISTS migrate_v12;

DELIMITER $$

CREATE PROCEDURE migrate_v12()
BEGIN
    IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES
               WHERE TABLE_SCHEMA = DATABASE()
                 AND TABLE_NAME = 'suggested_goals') THEN
